...
interface CommonMeaningDao {
     ...
     */
    @Query("SELECT * FROM CommonMeaning WHERE Id = :id")
     ...
     */
    @Query("SELECT * FROM CommonMeaning WHERE symbolId = :symId")
    fun getMeaningBySymbolId(symId: Int): Flow<List<CommonMeaning>>
